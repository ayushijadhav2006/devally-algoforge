...
export default function ActivitiesReports({ timeFrame, dateRange, activitiesData }) {
    ...
    if (activitiesData && activitiesData.length > 0) {
      ...
      const upcoming = activitiesData.filter(activity => {
        const activityDate = activity.date ? 
          (activity.date.toDate ? activity.date.toDate() : new Date(activity.date)) : 
          new Date();
      ...
      const totalParticipants = activitiesData.reduce(
        (sum, activity) => sum + (activity.registeredCount || activity.participants || 0), 
        0
      );
      
      // Top activities by participation
      const topActivities = [...activitiesData]
        .sort((a, b) => (b.registeredCount || b.participants || 0) - (a.registeredCount || a.participants || 0))
        .slice(0, 3)
        .map(activity => ({
          name: activity.name,
          date: activity.date,
