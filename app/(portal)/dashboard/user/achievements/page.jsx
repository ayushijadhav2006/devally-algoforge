...
export default function UserAchievementsPage() {
  ...
  const checkAndAwardAchievements = async (userId, userData) => {
    try {
      const achievementsRef = collection(db, "achievements");
      const userAchievementsRef = doc(
        db,
        "users",
        userId,
        "achievements",
      ...
      for (const achievement of achievements) {
        ...
        if (!userAchievements[achievementId]) {
          ...
          switch (achievement.type) {
            case "DONATION_COUNT":
              progress = userData.donatedTo?.length || 0;
              isEarned = progress >= achievement.requirement;
              break;
            case "DONATION_AMOUNT":
              progress =
                userData.donatedTo?.reduce(
              ...
              break;
            case "ACTIVITY_COUNT":
              progress = userData.participations?.length || 0;
              isEarned = progress >= achievement.requirement;
              break;
            case "VOLUNTEER_HOURS":
              progress = userData.volunteerHours || 0;
              isEarned = progress >= achievement.requirement;
              break;
          ...
          if (isEarned) {
            ...
            await sendNotificationToUser(userId, "ACHIEVEMENT_UNLOCKED", {
            ...
            // Check for level up
            const totalAchievements = Object.keys(userAchievements).length + 1;
            const newLevel = Math.floor(totalAchievements / 5) + 1;
            ...
            if (newLevel > (userData.level || 1)) {
              ...
              await sendNotificationToUser(userId, "LEVEL_UP", {
    ...
    <div className="container mx-auto py-8 px-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold">
              {translations.your_achievements || "Your Achievements"}
            </h1>
            <p className="text-muted-foreground mt-1">
              {translations.track_impact ||
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="md:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-xl">
              {translations.your_impact_level || "Your Impact Level"}
            </CardTitle>
            <CardDescription>
              {translations.level || "Level"} {userLevel?.level}:{" "}
              {userLevel?.name}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">
                  ...
                  {nextLevel
                    ...
                    : `${userLevel?.pointsRequired}+`}{" "}
                  {translations.points || "points"}
                </span>
                <span className="text-sm font-medium">
                  {nextLevel
                    ? `${translations.next || "Next"}: ${nextLevel.name}`
                    : translations.max_level_reached || "Max Level Reached!"}
                </span>
              </div>
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                {nextLevel && (
                  <span>
                    {translations.level || "Level"} {nextLevel.level}
                  </span>
                )}
              </div>
            </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xl">
              {translations.your_stats || "Your Stats"}
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Tabs defaultValue="earned" className="space-y-4">
        <TabsList>
          <TabsTrigger value="earned">
            ...
            {translations.earned_badges || "Earned Badges"} (
            {earnedBadges.length})
          </TabsTrigger>
          <TabsTrigger value="available">
            <Star className="mr-2 h-4 w-4" />
            {translations.available_badges || "Available Badges"} (
        ...
        <TabsContent value="earned" className="space-y-4">
          {earnedBadges.length === 0 ? (
            <Card>
              <CardHeader>
                <CardTitle>
                  {translations.no_badges_yet || "No Badges Yet"}
                </CardTitle>
                <CardDescription>
                  {translations.start_supporting_ngos ||
                ...
                <Card key={badge.id} className="overflow-hidden">
                  <CardHeader className="pb-2 flex flex-row items-center gap-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-xs">
                        {translations.earned_on || "Earned on"}{" "}
                        {badge.dateAwarded
                          ? new Date(
                              badge.dateAwarded.toDate
                                ? badge.dateAwarded.toDate()
                                : badge.dateAwarded
                            ).toLocaleDateString()
                          : translations.unknown_date || "Unknown date"}
                      </CardDescription>
                    </div>
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardFooter className="pt-0">
                    ...
                    >
                      +
                      {(ACHIEVEMENTS &&
                        ACHIEVEMENTS[badge.id.toUpperCase()]?.points) ||
                        0}{" "}
                      {translations.points || "points"}
                    </Badge>
                  </CardFooter>
        ...
        <TabsContent value="available" className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {pendingBadges.map((achievement) => (
              <Card
                key={achievement.id}
                className="overflow-hidden opacity-70 border-dashed hover:opacity-100 transition-opacity"
              >
                <CardHeader className="pb-2 flex flex-row items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-xs">
                      {translations.not_earned_yet || "Not earned yet"}
                    </CardDescription>
                  </div>
                </CardHeader>
                <CardContent>
                ...
                <CardFooter className="pt-0">
                  <Badge variant="outline">
                    +{achievement.points} {translations.points || "points"}
                  </Badge>
                </CardFooter>
              </Card>
            ))}
        ...
        <TabsContent value="levels" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>
                {translations.level_progression || "Level Progression"}
              </CardTitle>
              <CardDescription>
                {translations.advance_through_levels ||
                ...
                {LEVELS &&
                  LEVELS.map((level) => (
                    <div key={level.level} className="relative">
                      <div
                        className={`flex items-center p-3 rounded-lg ${userLevel?.level >= level.level ? "bg-primary/10" : "bg-muted"}`}
                      >
                        <div
                          className={`rounded-full p-2 mr-3 ${userLevel?.level >= level.level ? "bg-primary/20" : "bg-muted/50"}`}
                        >
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            ...
                            {translations.points_required || "points required"}
                          </p>
                        </div>
      ...
      <div className="mt-8">
        <h2 className="text-xl font-bold mb-4">
          {translations.how_to_earn_more || "How to Earn More Points"}
        </h2>
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">
                  {translations.purchase_merchandise || "Purchase Merchandise"}
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {translations.buy_products_message ||
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">
                  {translations.make_donations || "Make Donations"}
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {translations.donate_campaign_message ||
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">
                  {translations.join_activities || "Join Activities"}
                </CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {translations.participate_ngo_activities ||
