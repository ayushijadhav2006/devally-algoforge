    ...
    return (
      <div className="min-h-screen flex justify-center items-center bg-gradient-to-b from-gray-50 to-white">
        ...
        >
          ...
          <p className="font-semibold text-2xl text-emerald-600">{translations.loading_ngos || "Loading NGOs..."}</p>
        </motion.div>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
        ...
        >
          ...
          >
            ...
            >
              {translations.make_difference || "Make a Difference Today"}
            </motion.span>
            <motion.h1
            ...
            >
              {translations.discover_support || "Discover & Support"} <span className="text-emerald-400">{translations.ngos || "NGOs"}</span>
            </motion.h1>
            <motion.p
            ...
            >
              {translations.connect_organizations || "Connect with organizations making a difference in the world and find meaningful ways to contribute"}
            </motion.p>
            <motion.div
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              ...
              >
                {translations.explore_ngos || "Explore NGOs"} <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button
                variant="outline"
        ...
        <div className="container mx-auto py-12 px-4" id="ngo-list">
          ...
          >
            <h2 className="text-2xl font-bold mb-6 text-gray-800">{translations.find_cause || "Find Your Cause"}</h2>
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 w-full">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                <Input
                  type="text"
                  placeholder={translations.search_ngos_placeholder || "Search NGOs by name..."}
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white h-12 rounded-lg shadow-md hover:shadow-lg transition-all">
                    ...
                    {translations.filter_category || "Filter by Category"}
                    <ChevronDown className="h-4 w-4 ml-1" />
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{translations.filter_ngos_category || "Filter NGOs by Category"}</DialogTitle>
                    <DialogDescription>{translations.select_category || "Select a category to filter the NGO list"}</DialogDescription>
                  </DialogHeader>
              ...
              <div className="flex items-center space-x-2 bg-gray-100 p-1 rounded-lg">
                ...
                >
                  <Grid className="h-4 w-4 mr-1" /> {translations.grid || "Grid"}
                </Button>
                <Button
                ...
                >
                  <ListIcon className="h-4 w-4 mr-1" /> {translations.list || "List"}
                </Button>
              </div>
            </div>
            ...
            {selectedCategory !== "All" && (
              ...
              >
                <Badge className="bg-emerald-600 text-white py-1 px-3">
                  {translations[selectedCategory.toLowerCase()] || selectedCategory}
                  <button onClick={() => setSelectedCategory("All")} className="ml-2 hover:text-gray-200">
                    <X className="h-3 w-3" />
                  </button>
                </Badge>
                <span className="ml-2 text-sm text-gray-500">
                  {filteredNGOs.length} {translations.ngos_found || "NGOs found"}
                </span>
              </motion.div>
            )}
          </motion.div>
          ...
          {filteredNGOs.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 bg-white rounded-2xl shadow-lg">
              ...
              <h3 className="text-xl font-semibold mb-2">{translations.no_ngos_found || "No NGOs Found"}</h3>
              <p className="text-gray-500 max-w-md mx-auto">
                {translations.no_ngos_message || "We couldn't find any NGOs matching your search criteria. Try adjusting your filters or search term."}
              </p>
              <Button
                    ...
                    <Card className="overflow-hidden h-full bg-white border-0 shadow-lg rounded-xl hover:shadow-xl transition-all duration-300">
                      <div className="relative h-56 overflow-hidden">
                        <Image
                          ...
                          alt={ngo.ngoName || translations.ngo_logo || "NGO Logo"}
                          layout="fill"
                          objectFit="cover"
                        ...
                        <div className="absolute bottom-3 left-3">
                          <Badge className="bg-emerald-600/90 hover:bg-emerald-600 text-white font-medium">
                            {translations[ngo.category?.toLowerCase()] || ngo.category || translations.ngo || "NGO"}
                          </Badge>
                        </div>
                      </div>
                          ...
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-48">
                                ...
                                >
                                  {translations.donate_money || "Donate Money"}
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => handleDonate("resources", ngo.id)}
                                  className="cursor-pointer hover:bg-emerald-50"
                                >
                                  {translations.donate_resources || "Donate Resources"}
                                </DropdownMenuItem>
                                <DropdownMenuItem
                                  onClick={() => handleDonate("ethereum", ngo.id)}
                                  className="cursor-pointer hover:bg-emerald-50"
                                >
                                  {translations.donate_ethereum || "Donate via Ethereum"}
                                </DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </motion.div>
                      ...
                      <CardContent className="pb-4">
                        <div className="space-y-3 text-sm text-gray-600">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="truncate">{ngo.email || translations.no_email || "No email provided"}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Phone className="h-4 w-4 text-emerald-600" />
                            <span>{ngo.phone || translations.no_phone || "No phone provided"}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <MapPin className="h-4 w-4 text-emerald-600" />
                            <span className="truncate">{ngo.location?.address || translations.no_location || "No location provided"}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Users className="h-4 w-4 text-emerald-600" />
                            <span>{ngo.memberCount || 0} {translations.members || "members"}</span>
                          </div>
                        </div>
                      </CardContent>
